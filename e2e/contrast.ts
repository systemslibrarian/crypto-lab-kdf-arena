 ...
 *    files the node under `incomplete`. This palette builds four of its
 *    surfaces out of `rgba()`: `--accent-bg`, `--warn-bg`, `--social-bg` and
 *    the two `--*-border` tints. The hero aside, the salt callout, the
 *    schematic boxes and both preset buttons all sit on one of them, so their
 *    text composites onto whatever is behind rather than onto a flat colour
 *    axe can read.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `.run-btn:hover` renders its
 *    whole subtree at `opacity: .85` and `.rig-token-idle` at `.6`.
 *
 ...
 * nothing on screen has. None of the three is exercised by this lab as it
 * stands — every figure here is built from `<div>`s and `overflow: hidden`
 * clips rather than scrolls — and all three are kept because they are cheap,
 * silent when unused, and each is a case a later edit would reintroduce:
 *
 ...
 *    Its rect sits outside every ancestor's box, so the walk finds nothing
 *    behind it and falls through to white, inventing a failure.
 *
 ...
 *    SVG on this page is the shared header's icon set, and none of it carries
 ...
 *    composite those labels onto the svg's own transparent background and
 *    report a ratio nothing on screen has. So for SVG text, any earlier sibling
 *    shape whose box contains the text box is composited first.
 *
 * WHAT THIS FILE CANNOT SEE. Every oracle here walks ELEMENTS THAT OWN TEXT.
 * The memory grids, the attacker-rig lanes, the timing bars and the control
 * boundaries are none of those, and `aria-hidden` output — `.bar-value` carries
 * the millisecond figure and is hidden from the tree because `role="meter"`
 * already announces it — is skipped by both this walk and axe. Those are
 * hand-measured from screenshot pixels and their fixes live in `src/style.css`.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex would nearly cover this page — its own
     * surfaces are plain `rgba()` — but the shared header composes its border
     * and ink with `color-mix(in srgb, var(--accent) 38%, transparent)`, and
     ...
     * element and badly wrong for a page-sized wash, where a glow in one corner
     * would become the backdrop for a footer a full page-height below. So each
     * gradient is *sampled* at the text's real location: linear by projecting
     * onto the gradient line, radial by distance from the centre over the
     * farthest-corner radius. This lab's stylesheet declares no gradients at
     * all — every surface is a flat token or an `rgba()` tint — so this path is
     * currently inert, and it is kept so that adding one cannot silently go
     * unmeasured. A non-gradient layer (`url()`, `none`) is unmeasurable and
     * paints nothing.
     */
     ...
     *
     * A benchmarked page here holds four result cards, four memory grids of 144
     * cells each, 24 attacker-rig lane tokens and two bar charts, and every text
     * node in them walks the same handful of ancestors up to `.results`,
     * `main` and `body`. Without the caches the pass re-reads the same computed
     ...
     * This is live here: `.section-heading` is exactly `clip: rect(0, 0, 0, 0)`,
     * and it carries the headings that give the results region its structure.
     * Without this guard the walk composites that text against whatever surface
     * it happens to sit on and INVENTS failures for text nobody can see.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has two disclosures, the jargon glossary and
      // the cost-parameter panel, and the gate opens both by clicking their
      // summaries and scans each state.
      ...
      // `top: -3rem`, and the lab's own `.skip-link` at
      // `transform: translate(-50%, -150%)`, each sliding into view only on
     ...
     * be stricter than axe on decorative content. Honour the same boundary — but
     * know what it costs here. `.bar-value` carries the millisecond figure and
     * is `aria-hidden` because the `role="meter"` beside it already announces
     * the same number through `aria-valuetext`; the loop arrow and the spinner
     * glyph are hidden for the same reason. All of them are skipped by BOTH
     * oracles, so they are hand-measured from screenshot pixels instead. That
     * is a blind spot this file has, stated rather than papered over.
     */
